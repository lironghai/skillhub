...
# /.well-known/, /runtime-config.js, /contextforge/). Generating `location ^~ /api/` would
...
if [ "$SKILLHUB_WEB_BASE_PATH" != / ]; then
  ...
  case "$first_segment" in
    api|oauth2|login|assets|registry|nginx-health|.well-known|runtime-config.js|contextforge)
