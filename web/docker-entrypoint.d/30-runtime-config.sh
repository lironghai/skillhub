...
: "${SKILLHUB_MCP_ENABLED:=true}"
: "${SKILLHUB_WEB_MCP_CONTEXT_FORGE_ENABLED:=false}"

if [ "$SKILLHUB_MCP_ENABLED" != "true" ]; then
  SKILLHUB_WEB_MCP_CONTEXT_FORGE_ENABLED=false
fi
...
export \
  ...
  SKILLHUB_WEB_AUTH_DIRECT_PROVIDER \
  SKILLHUB_WEB_MCP_CONTEXT_FORGE_ENABLED \
  SKILLHUB_WEB_AUTH_SESSION_BOOTSTRAP_ENABLED \
  SKILLHUB_WEB_AUTH_SESSION_BOOTSTRAP_PROVIDER \
  SKILLHUB_WEB_AUTH_SESSION_BOOTSTRAP_AUTO

# Generate runtime-config.js
envsubst '${SKILLHUB_WEB_API_BASE_URL} ${SKILLHUB_PUBLIC_BASE_URL} ${SKILLHUB_WEB_AUTH_DIRECT_ENABLED} ${SKILLHUB_WEB_AUTH_DIRECT_PROVIDER} ${SKILLHUB_WEB_MCP_CONTEXT_FORGE_ENABLED} ${SKILLHUB_WEB_AUTH_SESSION_BOOTSTRAP_ENABLED} ${SKILLHUB_WEB_AUTH_SESSION_BOOTSTRAP_PROVIDER} ${SKILLHUB_WEB_AUTH_SESSION_BOOTSTRAP_AUTO}' \
