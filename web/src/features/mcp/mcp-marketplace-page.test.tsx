        ...
        items: [
          {
            ...
            logoUrl: 'https://contextforge.example/github.png',
...
describe('McpMarketplacePage', () => {
  ...
  it('renders the open-source logo returned by the catalog API', () => {
    const catalog = mcpCatalogHooks.useMcpCatalog()
    const html = renderToStaticMarkup(<OpenSourceServerCard server={catalog.data.items[0]} />)

    expect(html).toContain('src="https://contextforge.example/github.png"')
  })
  ...
  it('shows MCP bearer authentication without requiring an unrelated upstream credential', () => {
    const server = mcpCatalogHooks.useMcpInternalServers().data.items[0]
    const html = renderToStaticMarkup(<McpTokenUsageHint server={server} />)

    expect(html).toContain('Authorization: Bearer &lt;YOUR_ACCESS_TOKEN_HERE&gt;')
    expect(html).not.toContain('X-Upstream-Authorization')
    expect(html).toContain('mcpMarketplace.tokenScopeHint')
  })

  it.each(['bdc4_group', 'service "quoted" \\ name'])('renders pasteable HTTP configuration for %s with a copy action', (name) => {
    const server = { name, streamableHttpUrl: 'https://example.com/servers/actual-id/mcp?x=1&y=2' }
    const container = document.createElement('div')
    container.innerHTML = renderToStaticMarkup(<McpTokenUsageHint server={server} />)
    const json = container.querySelectorAll('pre code')[1]?.textContent ?? ''

    expect(JSON.parse(json)).toEqual({
      [name]: {
        type: 'http',
        url: 'https://example.com/servers/actual-id/mcp?x=1&y=2',
        headers: { Authorization: 'Bearer <YOUR_ACCESS_TOKEN_HERE>' },
      },
    })
    expect(container.querySelector('button')?.textContent).toBe('copyButton.copy')
  })
})
