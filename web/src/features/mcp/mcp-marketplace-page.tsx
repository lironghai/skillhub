        ...
        <div className="flex flex-col gap-3 sm:flex-row">
          <div className="relative flex-1">
            ...
            <Input
              ...
              className="h-14 pl-10 pr-[10.5rem] w-full"
            />
            {searchInput ? (
              <button
                type="button"
  ...
  return (
    <>
      ...
      <Dialog open={rowDetail === true} onOpenChange={(open) => {
        ...
        <DialogContent className="w-[min(calc(100vw-2rem),40rem)] max-h-[calc(100vh-2rem)] grid-cols-[minmax(0,1fr)] overflow-x-hidden overflow-y-auto p-0">
...
export function McpTokenUsageHint({ server }: { server: Pick<McpInternalServerItem, 'name' | 'streamableHttpUrl'> }) {
  const { t } = useTranslation()
  const [copied, copy] = useCopyToClipboard()
  const authorization = 'Bearer <YOUR_ACCESS_TOKEN_HERE>'
  const config = JSON.stringify({
    [server.name]: {
      type: 'http',
      url: server.streamableHttpUrl,
    ...
    <section className="mt-5 space-y-2 rounded-lg border bg-secondary/20 p-4">
      ...
      {server.streamableHttpUrl && (
        <div className="min-w-0 rounded-md bg-background">
          ...
          <pre className="overflow-x-auto p-3 text-xs leading-5 text-foreground"><code>{config}</code></pre>
        </div>
      )}
      <p className="text-xs leading-5 text-muted-foreground">{t('mcpMarketplace.tokenScopeHint')}</p>
    </section>
  )
}
...
export function OpenSourceServerCard({ server }: { server: McpCatalogItem }) {
  const { t } = useTranslation()
  return (
        ...
        <EmptyState
          ...
          description={
            search
              ...
              : t("mcpMarketplace.emptyDescription")
          }
          action={
            search ? (
