  ...
  return (
    <>
      ...
      <main ref={heroView.ref} className={`relative z-10 flex flex-col items-center pt-16 pb-20 px-4 md:pt-24 scroll-fade-up${heroView.inView ? ' in-view' : ''}`}>
        ...
        {/* Brand logo mark */}
        <div className="w-[44px] h-[44px] rounded-[12px] bg-brand-gradient flex items-center justify-center mb-5 shadow-[0_2px_8px_rgba(159,69,66,0.12)]">
        ...
        <div className="w-full max-w-[700px] mb-8">
          ...
          >
            <SearchIcon className="w-6 h-6 flex-shrink-0 mr-3" style={{ color: '#999' }} strokeWidth={1.5} />
            <input
              type="text"
        ...
        <div className="flex items-start justify-center gap-16 md:gap-24">
          {[
            ...
            <div key={stat.label} className="flex flex-col items-center">
              <span className="text-[38px] font-bold leading-none text-brand-gradient" style={{ textShadow: '0 2px 8px rgba(159,69,66,0.12)' }}>
                {stat.value}
              </span>
      ...
      {/* Features Section */}
      <section ref={featuresView.ref} className={`relative z-10 w-full pt-[96px] pb-20 md:pb-24 px-6 scroll-fade-up${featuresView.inView ? ' in-view' : ''}`} style={{ background: 'var(--bg-page, hsl(var(--background)))' }}>
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-14">
            <h2 className="text-3xl md:text-4xl font-bold tracking-tight mb-3" style={{ color: '#333' }}>
              {t('landing.whyTitle', { defaultValue: '为什么选择' })}{' '}
              <span className="text-brand-gradient" style={{ textShadow: '0 2px 8px rgba(159,69,66,0.12)' }}>HeroSkillHub</span>
            </h2>
            <p className="text-base md:text-lg max-w-2xl mx-auto leading-relaxed" style={{ color: '#666' }}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {features.map((feature, idx) => (
              <div
                ...
                style={{
                  borderColor: '#e5e5e5',
                  boxShadow: idx === 1 ? '1px 2px 16px 1px rgba(191,55,50,0.1)' : undefined,
                }}
              >
                <div className="w-12 h-12 rounded-[12px] flex items-center justify-center mb-6 mx-auto bg-brand-gradient/10" style={{ boxShadow: '0 2px 8px rgba(159,69,66,0.12)' }}>
                  <div className="text-brand-gradient">
                    {feature.icon}
                  </div>
                </div>
                <h3 className="text-lg font-semibold text-center mb-3" style={{ color: '#333' }}>
                  {feature.title}
                </h3>
      ...
      <section ref={popularView.ref} className={`relative z-10 w-full py-20 md:py-24 px-6 scroll-fade-up${popularView.inView ? ' in-view' : ''}`} style={{ background: 'var(--bg-page, hsl(var(--background)))' }}>
        <div className="max-w-6xl mx-auto space-y-6">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-[30px] font-bold tracking-tight mb-2" style={{ color: '#333' }}>
                {t('home.popularTitle')}
              </h2>
              <p className="text-[18px]" style={{ color: '#666' }}>{t('home.popularDescription')}</p>
            </div>
            <Button
              variant="ghost"
        ...
        <div className="max-w-6xl mx-auto space-y-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-[18px]" style={{ color: '#666' }}>{t('home.latestDescription')}</p>
            </div>
            <Button
              variant="ghost"
              className="text-[14px] font-medium"
