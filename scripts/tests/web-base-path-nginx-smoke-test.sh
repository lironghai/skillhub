...
if ! docker run -d --name "$name" \
    ...
    -e SKILLHUB_CONTEXT_FORGE_UPSTREAM=http://127.0.0.1:9 \
...
port=$(docker port "$name" 80/tcp | awk -F: 'END {print $NF}')
...
fixed_port=$(docker port "$fixed_name" 80/tcp | awk -F: 'END {print $NF}')
