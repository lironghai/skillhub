...
if [ "$(grep -Fc 'proxy_set_header X-Forwarded-Prefix $skillhub_forwarded_prefix;' "$ROOT_DIR/web/nginx.conf.template")" -ne 5 ]; then
  echo 'API, workbench SSE, OAuth, and .well-known proxy locations must forward the configured base path' >&2
  exit 1
fi
...
for bad in '/foo/../bar/' '/foo/./bar/' '/foo//bar/' '/no-trailing' 'foo/' \
           ...
           '/.well-known/' '/runtime-config.js/' '/contextforge/' '/api/nested/'; do
